    ...
    ,ui(new Ui::ResetDialog)
{
    ...
    connect(ui->email_Edit_3, &QLineEdit::editingFinished, this, [this](){
        checkEmailValid();
    });

    connect(ui->verify_lineEdit_4, &QLineEdit::editingFinished, this, [this](){
        checkPassValid();
    });


    connect(ui->pwd_Edit_5, &QLineEdit::editingFinished, this, [this](){
        checkVarifyValid();
    });
...
{
    //验证邮箱的地址正则表达式
    auto email = ui->email_Edit_3->text();
    // 邮箱地址的正则表达式
...
{
    auto pass = ui->verify_lineEdit_4->text();
    if(pass.isEmpty()){
...
void ResetDialog::on_sur_btn_clicked()
{
    ...
    json_obj["email"] = ui->email_Edit_3->text();
    json_obj["passwd"] = xorString(ui->pwd_Edit_5->text());
    json_obj["varifycode"] = ui->verify_lineEdit_4->text();
